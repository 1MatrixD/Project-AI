...
def answer_for(prompt: str) -> str:
    ...
    if "Материал бывает двух родов" in prompt:
        # Материал-уточнение: дополняем уже заведённую задачу и заводим одну новую.
        if "загружен как уточнение" in prompt:
            return json.dumps(
                {
                    "summary": "ТЗ с деталями по фиче А.",
                    "tasks": [
                        {"title": "Новая задача из ТЗ", "description": "То, чего на созвоне не было", "plan": []},
                    ],
                    "updates": [
                        {
                            "task_title": "Сделать фичу А",
                            "add": "Логика девяти игроков: расстановка 4-3-2, замены только в перерыве.",
                            "why": "ТЗ детализирует ту же фичу, отдельной задачей это не является",
                        },
                        {
                            "task_title": "Задачи с таким названием нет",
                            "add": "Дополнение-сирота",
                            "why": "проверка, что промах по названию не роняет обработку",
                        },
                    ],
                    "decisions": [],
                },
