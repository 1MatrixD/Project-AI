...
а ИИ должен сам разобраться, куда смотреть и как выполнять. Пайплайн `enrich_tasks`:
...
1. **RLM-исследование** по задаче: корень выбирает группы файлов по карте знаний,
   ...
   вероятная причина бага, аналогичный код «по образцу».
2. **Синтез задачи**: из фактов исследования + списка существующих задач (сделанных
   и открытых!) собирается детальное описание со ссылками на файлы и пошаговый план
   в стиле реальной работы инженера (диагностика → локализация → правка по аналогии →
   проверка). План — чеклист `{text, done}`.
3. **Связи**: `related_tasks` (duplicate/continuation/overlaps) и `duplicate_of` —
