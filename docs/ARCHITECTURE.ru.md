# Архитектура «Проекты ИИ»

## Общая схема

```
Браузер (Next.js 16, :3010)
   │  REST + SSE
   ▼
FastAPI (:8010) ──── JobRunner (встроенные фоновые воркеры)
   │        │                │
   │        │                ├─ index: скан → diff → граф → ИИ-анализ → синтез → worklog
   │        │                ├─ process_material: whisper/извлечение → задачи
   │        │                ├─ verify_tasks: ИИ-проверка выполненности
   │        │                └─ plugin_generate: плагин Claude Code
   │        │
   │        ├─ Postgres (docker): users, projects, files, chats, jobs,
   │        │                     tasks (канбан), worklog, materials, change_reports
   │        ├─ Neo4j (docker):    карта знаний (граф) + fulltext-поиск
   │        └─ Qdrant (docker):   векторный (семантический) поиск — эмбеддинги
   │                              файлов, материалов, соглашений (fastembed, локально)
   │
   └─ claude -p (Claude Code CLI, headless)
        │  --mcp-config → MCP-сервер projectai (stdio, backend/mcp_main.py)
        └─ инструменты: graph_search, graph_cypher, rlm_query, task_*, log_work…
```

Всё крутится локально (self-hosted): бэкенд имеет прямой доступ к каталогам проектов,
`claude -p` использует локальную авторизацию Claude Code, Whisper работает на GPU/CPU.

## Схема графа знаний (Neo4j)

Все узлы несут `project_id` и уникальный `uid` = `project_id|вид|идентификатор`.

| Узел | Что это | Ключевые связи |
|---|---|---|
| `Project` | корень проекта | `CONTAINS`, `HAS_COMPONENT`, `HAS_FEATURE`, `HAS_DOCUMENT`, `HAS_TASK`, `HAS_WORKLOG` |
| `Directory` | каталог | `CONTAINS` (вложенность) |
| `File` | файл: роль, summary, теги | `DEFINES → Entity`, `RELATES {type} → File` |
| `Entity` | класс/функция/эндпоинт/модель/экран/параметр | — |
| `Component` | компонент/сервис/фича из синтеза | `INCLUDES → File` |
| `Document` | материал (транскрипт, ТЗ) | `MENTIONS → File` |
| `Task` | задача канбана | `AFFECTS → File` |
| `WorkLog` | запись «что сделано» | `UPDATED → File` |

Полнотекстовый индекс `knowledge_fulltext` по name/path/summary/title плюс
семантический поиск по эмбеддингам (Qdrant, `services/vectors.py`) — вместе дают
гибридный `graph_search`: у каждого хита `match: fulltext | semantic | both`.
Эмбеддинги считаются локально (fastembed/ONNX, модель multilingual-MiniLM) при
ИИ-анализе файлов, обработке материалов и записи соглашений; недоступный Qdrant
не валит пайплайны (поиск деградирует до fulltext). `graph_cypher` — read-only
(записи отклоняются).

## Мультирепо (`services/roots.py`)

Проект может состоять из нескольких каталогов: основной (`root_path`, файлы без
префикса) и дополнительные (`meta.extra_roots: [{alias, path}]`, файлы с префиксом
`alias/` в реестре, графе, векторах и задачах). Алиас выбирается без конфликтов
с верхним уровнем основного каталога, поэтому любой путь разрешается однозначно.
ИИ-анализ идёт батчами per-root (cwd своего корня, пути в промпте локальные),
RLM подставляет для чужих корней абсолютные пути, watchdog наблюдает за всеми
корнями, git-импорт находит репозитории во всех каталогах. Удаление каталога
чистит его файлы из реестра, графа и векторного индекса.

## Пайплайн индексации (`services/indexer.py`)

1. **Скан** (`scanner.py`): обход каталога с игнор-листами (node_modules, .git, build…)
   **и `.gitignore`** — включая вложенные .gitignore подпроектов монорепо (pathspec);
   sha256-хэши (с переиспользованием по mtime+size), классификация kind.
2. **Diff**: added/modified/deleted → `change_reports` (то самое «что обновилось»).
3. **Граф структуры**: батчевые MERGE Project/Directory/File, удаление исчезнувших.
4. **Детект** (`detect.py`): тип проекта и стек по маркер-файлам (package.json, pubspec…).
5. **ИИ-анализ**: батчи по `AI_BATCH_SIZE` файлов → `claude -p` (модель `AI_MODEL`,
   обычно sonnet) с инструментом Read и строгим JSON-выводом: роль, summary, сущности,
   связи → граф. Бюджет — `AI_MAX_FILES_PER_RUN` за прогон; остаток дообрабатывается
   следующими «Обновить индекс».
6. **Синтез**: обзор проекта (архитектура, компоненты, бизнес-логика, конвенции,
   how-to) → `Project.summary`, `Component`-узлы, `project.meta.overview`.
7. **Плагин** перегенерируется со свежими знаниями.

Режимы: `initial` (при создании), `update` (изменения + продолжение бэклога анализа),
`reverify` (сброс всех меток → полный пере-анализ).

## RLM — Recursive Language Models (MIT)

Идея ([Zhang, Khattab]): при больших контекстах модель не должна глотать всё окно —
контекст лежит «в среде», модель исследует его программно и рекурсивно вызывает
саб-модели над фрагментами.

Реализация (`services/rlm.py` + MCP-инструмент `rlm_query`):

- **Среда** = каталог проекта + граф знаний + реестр файлов с ролями.
- **Корень** (план): получает выжимку графа, результаты fulltext-поиска и индекс файлов →
  выбирает группы файлов (до 4 групп × 12 файлов) под вопрос.
- **Под-вызовы**: изолированные `claude -p` с инструментом Read, каждый читает только
  свою группу и возвращает сжатый ответ.
- **Синтез**: корневой вызов собирает финальный ответ из под-ответов.
- **Глубина** (`RLM_MAX_DEPTH`): `1` — корень и один слой под-агентов; `2` и больше —
  под-агент, которому не хватило назначенных файлов, дописывает блок «НУЖНО УТОЧНИТЬ»,
  и по каждому вопросу рекурсивно запускается тот же пайплайн; `0` — без ограничения.
  Ветвление на уровень задаёт `RLM_BRANCHING`, общий предохранитель — `RLM_MAX_NODES`
  (сколько вложенных исследований допустимо за прогон) и жёсткий потолок глубины.
  Семафор параллельности общий на всё дерево, поэтому глубина не размножает процессы.

Чат-ассистент — сам корневой RLM-агент: системный промпт учит его не читать десятки
файлов в свой контекст, а звать `rlm_query`. Эндпоинт `POST /api/projects/{id}/ask` —
RLM-вопрос без чата.

## Цикл актуальности знаний

```
ИИ/пользователь делает работу
   └─ task_done(отчёт, файлы) / log_work(...)   ← MCP или UI
        └─ WorkLogEntry (synced=False) копится; бейдж в шапке показывает счётчик
             └─ ручное «⟳ Индекс» (или request_reindex)
                  ├─ скан diff → изменённые файлы → пере-анализ → граф
                  ├─ WorkLog/Task-узлы → связи UPDATED/AFFECTS с файлами
                  ├─ уборка Task-узлов удалённых задач (сироты)
                  └─ ChangeReport («что обновилось»)
```

Удаление задачи чистит её узел сразу, но граф бывает недоступен, поэтому индекс
дополнительно подметает сироты. Это не косметика: заголовок Task-узла — формулировка
задачи, он лежит в полнотекстовом индексе и на похожий вопрос попадает в топ-15
выдачи, по которой RLM выбирает файлы для чтения (`rlm.py`), отнимая места у файлов.

Обновление карты — ручное: автозапуск `knowledge_update` после каждой задачи
конкурировал за ИИ-слоты с проработками при пакетной работе. Тип джобы оставлен
для истории; вся работа (скан + учёт worklog) делается внутри `index_project`.

Плюс `verify_tasks`: для каждой открытой задачи ИИ (Read/Grep/Glob по коду) выносит
вердикт yes/partial/no — «yes» помечается done с отчётом и файлами-доказательствами.

## RLM-проработка задач (`services/task_enrich.py`)

Ключевой сценарий: после созвона задачи короткие («починить кнопку удаления интеграции»),
и система готовит по ним досье для исполнителя — человека или ИИ-агента, который сам
решит, как делать. Решение сознательно НЕ предписывается: план, построенный моделью на
этапе проработки, становится потолком для более сильного исполнителя, а факты — полом.
Пайплайн `enrich_tasks`:

1. **RLM-исследование** по задаче: корень выбирает группы файлов по карте знаний,
   под-агенты читают их и возвращают факты — точные пути, как реализовано сейчас,
   вероятная причина бага, аналогичный код «по образцу», тестовый контур рядом.
2. **Синтез досье**: из фактов исследования + списка существующих задач (сделанных
   и открытых!) собирается: `reading` (как понята задача), `hypothesis` (вероятная
   причина с уверенностью), `where_to_look` (места + что там проверить), `reference`
   (где рядом сделано правильно), `how_to_verify` (что должно стать правдой и каким
   тестом/командой проверяется). Императивы «добавь/поменяй» запрещены промптом;
   поле `plan` проработка больше не заполняет.
3. **Развилки и последствия**: `open_questions` — то, что должен решить человек (два
   защитимых варианта с последствиями; пустой список = «выбор очевиден»), `impact` —
   что заденет правка помимо места изменения: пороги, автоматы, контракты между слоями.
   Без этих полей модель молча решала за продукт — форма ответа диктует поведение.
4. **Доисследование**: синтез отдельным полем `unresolved` перечисляет, чего не нашёл
   в коде; если список непуст, по нему запускается один узкий RLM-заход и описание
   пересобирается. Иначе в задаче оседает догадка «наверное, где-то есть».
5. **Связи**: `related_tasks` (duplicate/continuation/overlaps) и `duplicate_of` —
   чтобы задачи с созвонов не дублировались и продолжения находили родителей;
   файлы задачи попадают в граф связями `AFFECTS`.

Запускается автоматически после извлечения задач из материалов, кнопками в UI
(«🧠 Проработать» на задаче, «Проработать новые (RLM)» на доске), MCP-инструментом
`task_enrich`. Извлечение задач из новых материалов тоже видит существующие задачи
и не создаёт дубликаты.

## Чат (`routers/chats.py`)

- `claude -p` со стримингом `stream-json` → SSE (`delta`, `tool`, `done`, `error`).
- Сессии: `--resume <session_id>` — контекст диалога живёт в Claude Code.
- Модель per-chat: `--model opus|sonnet|haiku` (**opus = Opus 5 по умолчанию**).
- Reasoning per-chat: none/low/medium/high → `MAX_THINKING_TOKENS` (0/4k/12k/32k).
- Инструменты: Read/Grep/Glob/LS + весь MCP `projectai`. Запись в файлы чату не дана —
  изменения проекта фиксируются через задачи/worklog.

## MCP-сервер (`app/mcp/server.py`)

Stdio-сервер (FastMCP), запускается самим Claude Code (из чата или плагина).
Работает через HTTP API бэкенда с **сервисным JWT**, ограниченным одним проектом.
Инструменты: `project_overview`, `graph_search`, `graph_cypher`, `list_files`,
`list_documents`, `read_document`, `rlm_query`, `task_list/create/update/move/done`,
`log_work`, `request_reindex`.

## Плагины Claude Code (`services/plugin_gen.py`)

`data/plugins/<slug>/`: `.claude-plugin/plugin.json`, `.mcp.json` (тот же MCP-сервер),
`skills/architecture`, `skills/business-logic`, `skills/project-workflow` — генерируются
из карты знаний при каждой индексации. `data/plugins/.claude-plugin/marketplace.json` —
маркетплейс всех проектов: `claude plugin marketplace add <path>` один раз, дальше
`claude plugin install <slug>@projectai`.

## Материалы (`services/materials.py`)

- Аудио/видео → faster-whisper (large-v3, CUDA float16 с фолбэком на CPU int8),
  таймкоды в транскрипте.
- pdf → pypdf, docx → python-docx, xlsx → openpyxl, txt/md/json — напрямую
  (кодировка через charset-normalizer).
- Текст → ИИ-выжимка + извлечение задач (source=meeting/doc) → канбан + Document-узел.
- Заметка своими словами (`POST /materials/note`) — тот же материал, только набранный
  руками. Отдельной механики под неё нет намеренно.

### Материал-уточнение

Созвон даёт скелет задач, а ТЗ или заметка, пришедшие позже, — логику, без которой
их не сделать. Раньше это терялось: промпт извлечения велел пропускать всё, что
уточняет уже заведённую задачу, и вторая половина замысла оседала в файле.

Материал можно загрузить как уточнение к более раннему (`?clarifies=<id>`,
хранится в `material.meta`). Тогда:

1. Задачи, рождённые тем материалом (`task.extra.from_material`), уходят в промпт
   с ПОЛНЫМ описанием — по 120 символов из общего списка мало, чтобы решить
   «дополнить или завести новую».
2. Контракт извлечения знает глагол «дополнить»: `updates` с дословным названием
   задачи. Промах по названию логируется и пропускается — задача не портится.
3. Дополнение ложится в `task.extra.clarifications`, а НЕ в описание: описание
   проработка пересобирает целиком и дописанный туда текст затёрла бы.
4. Флаг `enriched` снимается, задача уходит на переработку — досье пересобирается
   уже с новой логикой.

Там же живут `task.extra.notes` — заметки владельца задачи. Оба блока проработка
читает (`human_input_block`) и никогда не перезаписывает: это прямая речь людей,
которая весомее выводов из кода.

## Безопасность

- JWT (HS256), bcrypt; сервисные токены скоупятся project_id.
- `graph_cypher` — только чтение; чат без Write/Bash.
- Секреты в `.env` (не в гите); сервисный токен не отдаётся в API проекта.
- Система рассчитана на локальный/доверенный контур (fs-браузер отдаёт каталоги
  владельцу аккаунта).

## Локализация

- **Интерфейс**: next-intl без префиксов в URL. Локаль берётся из cookie, которую ставит
  переключатель RU / EN, а при первом визите — из `Accept-Language` браузера; словари лежат
  в `frontend/messages/*.json`. Даты и единицы размера следуют локали.
- **Сообщения API**: фронтенд шлёт язык интерфейса в `Accept-Language`; чистый ASGI-middleware
  кладёт его в контекстную переменную, и `i18n._()` подбирает перевод (gettext-стиль: ключ
  и запасной вариант — русская строка).
- **ИИ-контент и фоновые работы**: `AI_LANGUAGE` (`en` / `ru`) — в каждом шаблоне промпта
  подставляется пометка языка, к системному промпту каждого вызова `claude -p` дописывается
  директива о языке, и та же настройка управляет сообщениями о ходе фоновых работ, экспортом
  карты знаний и сгенерированными скиллами плагина.
